...
import shacs.dmotion.com.shacs.R;
...
public class MultiDirectionSlidingDrawer extends ViewGroup {
    ...
    private static final float MAXIMUM_MAJOR_VELOCITY = 220.0f;
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (mTracking || mAnimating) {
            ...
            if (cache != null) {
                ...
                if (mInvert) {
                    ...
                    nMaxWidth = mContent.getMeasuredWidth();
                } else {
            ...
            if (mOnAlphaChangeListener != null)
                mOnAlphaChangeListener.onAlphaChanged(0.7f);
            return;
        }
        ...
        if (mOnAlphaChangeListener != null) {
            if (mExpanded)
                mOnAlphaChangeListener.onAlphaChanged(0.7f);
            if (nMaxWidth == 0 || mHandle.getLeft() == 0)
                mOnAlphaChangeListener.onAlphaChanged(0.0f);
            else {
                float fPercent = (float) handle.getLeft() / (float) nMaxWidth;
                if (fPercent >= 0.3)
                    fPercent -= 0.3;
                else
                    fPercent = 0;

                mOnAlphaChangeListener.onAlphaChanged(fPercent);
            }
        }
    }
...
//						}
    ...
    private void incrementAnimation() {
        long now = SystemClock.uptimeMillis();
        float t = (now - mAnimationLastTime) / 1700.0f; // ms -> s
    ...
    private void closeDrawer() {
        ...
        mHandle.setBackgroundResource(0);
        mHandle.setAlpha(0.0f);
